...
{top_actions}
prepare_prompt: {prep_text}
- prepare_prompt (这个是在执行当前prompt之前，已经执行过的prepare_prompt,现在的图像就是刚才执行完成prepare_prompt的结果，如果是no need 那就是没有prepare这一步)
【外部历史记录文件（必须读取）】
history_jsonl_path: {history_path}
1) 先调用“文件读取工具”（read_file等）读取 history_jsonl_path 的完整内容。
2) 只在脑子里检索与 current_subtask 相同的记录行，重点看：
   - prompt2
   - outcome (succeeded/failed)
   - prepare_prompt (这个是历史记录中针对各个任务的执行prepara_prompt之后执行promp2的情况)
3) 不允许把文件内容原样输出、不允许复述日志；只把这些历史用于你内部决策。
4) 这里存放以往解决任务的promt以及结果信息，可以参考，可以知道针对这个任务，都用什么样子的prompt可以解决
====================
...
你是 CALVIN 测评中的“策略改进者”。你不能改判定函数，只能选择下一步要执行的指令（prompt），用试验去提升成功率。
  ...
  (3) 利用image understanding的MCP工具去分析这个路径的图片/root/autodl-tmp/RLinf/RGB/RGB.png
      并在脑子里记录图像要点（目标块位置、最近的地标 object、led 的相对位置），
      然后再生成最终 prompt。

【场景布局先验（仅用于帮助解读图像；最终以 analyze_image 为准）】
- 画面有机械臂与桌台（play table）。
- 桌面左侧靠前有一个 button，用于控制 led。button是纯黑色的,你要确定好位置
...
2) 地标词白名单（禁止发明）
- 你只能使用以下地标物体名（逐字）：led，尽量用led，因为table的范围太大了，pi05会不知道放在哪里
- 不允许出现任何其他新物体名、新地点名、新区域名（如 corner/edge/center/handle 等若不是白名单就不要用）。

3) 方位词白名单（用来表达落点，不要精细到不存在的位置）
- 你只能使用这些相对方位词来描述落点：to the left of/ to the right of  不能用其他的 
...
- 如果是block这一类，要考虑移动他们的话，需要用上面的动词选一个，还需要一个目标移动到什么地标物体的哪个方位，比如就使用 led，看看子任务要求是什么，选择一个最近的可以达到目的地标物体，要加介词，不要连接符
- prompt不能太长，只能用一个动词，根据上面的技能的instruction可以完成的子任务，找一个动词可以一次完成我们这个任务即可，尽量不要分两个动词
6) 简洁性
- prompt 必须是一句话，尽量短（建议 ≤ 12 个英文单词），不要写多步长句或多段，不能是空的。
...
额外硬性要求：
- 输出不能是空的
- 只输出这一行（不要解释、不要分析、不要 JSON、不要多行，不要是乱码，工具调用等，检查好输出）
- prompt 必须是一句话，尽量短（要≤ 12 个英文单词）
...
async def main(args):
    ...
    prompt_log_path = "/root/autodl-tmp/RLinf/prompt_trials.json"
    ...
    pre_rule_agent_ctx=await miniagent_setup_once()
    pre_rule_agent= miniagent_make_agent(pre_rule_agent_ctx, Path("./miniagent_workspace"))


    drawer_agent_ctx=await miniagent_setup_once()
    drawer_agent= miniagent_make_agent(drawer_agent_ctx, Path("./miniagent_workspace"))
    ...
    for i, (initial_state, task_sequence) in enumerate(tqdm.tqdm(task_definitions)):
     ...
     #   logger.info(f"sub tasks list: {init_resp}")  
        ...
        analyze_agent.messages.clear()
        thinking_agent.messages.clear()
        pre_rule_agent.messages.clear()
        #尝试agent调用pi05
        ...
        for j, subtask in enumerate(task_sequence):
            ...
            PRE_RULE_PROMPT = f"""
                        你是“后续依赖检查器（Forward Dependency Checker）”。
                        当前子任务刚执行完成。你只判断：后续任务是否依赖于“当前完成的 slider 状态”或“抽屉打开状态”。
                        若依赖：只输出当前子任务名字（原样）。
                        否则：输出 NONE。

                        【硬性规则】
                        1) 只能用输入信息判断：DONE_SUBTASKS / CURRENT_SUBTASK / REMAINING_SUBTASKS。
                        2) 禁止编造任务名；最终输出只能是 CURRENT_SUBTASK 或 NONE。
                        3) 禁止输出解释、标点、空行或多余文本；只输出一行。
                        ...
                        【判定规则】
                        A) 如果 CURRENT_SUBTASK 不是以下之一：
                        - move_slider_left
                        - move_slider_right
                        - open_drawer
                        则输出 NONE。

                        B) 如果 CURRENT_SUBTASK 是 move_slider_left 或 move_slider_right：
                        只要 REMAINING_SUBTASKS 中出现任意 slider 相关任务（名字包含 "slider"），输出 CURRENT_SUBTASK；
                        否则输出 NONE。

                        C) 如果 CURRENT_SUBTASK 是 open_drawer：
                        只要 REMAINING_SUBTASKS 中出现任意 drawer 相关任务（名字包含 "drawer"），输出 CURRENT_SUBTASK；
                        否则输出 NONE。

                        【输出】
                        只输出一行：CURRENT_SUBTASK 或 NONE
                        """
            ...
            if subtask.startswith("push_") and  subtask.endswith("_right"):
                                print("33333333333")
                                PREPARE_PROMPT = f"""
                                    你是 PREPARE 决策 agent：在 start_info 之前决定“是否需要先 reposition 目标物体到更高成功率的起始侧”。

                                    输入：
                                    - subtask: "{subtask}"
                                    - history_jsonl_path: "{prompt_log_path}"

                                    强制流程：
                                    1) 必须先调用文件读取工具读取 history_jsonl_path 全文；若读取失败/文件不存在/为空：立即输出 no need 并结束。
                                    2) 只在脑中从历史中筛选“同类任务”：
                                    - 若 subtask 包含 "push_" 且包含 "_right" => 同类为 push_*_right
                                    - 若 subtask 包含 "push_" 且包含 "_left"  => 同类为 push_*_left
                                    不要参考非同类的，参考和自己的物体名字一样的并且动作和目标方位都一致的样本，看看之前遇到这个subtask都是怎么成功解决的
                                    注意：历史里的准备字段名是 prepare_prompt；值为 "no need" 表示未做准备。
                                    
                                    3) 你只负责：要不要准备 + 准备把目标物体放到 led 的哪一侧。不要做障碍分析，不要输出任何历史内容。

                                    输出（只能二选一，严格遵守）：
                                    - 若不需要准备：输出 exactly "no need"
                                    - 若需要准备：只输出 1 行英文指令，格式必须满足：
                                    a) 只能用动词 "place"
                                    b) 必须包含目标物体（例如 "the red block"）
                                    c) 目标位置只能是 "to left of the led" 或 "to right of the led"
                                    d) 单句 ≤ 12 个英文单词
                                    e) 只做 reposition，不要包含 push/rotate/lift/turn 等最终任务动作
                                    禁止输出：解释/分析/标题/空行/JSON/工具调用/任何多余字符，输出之前再次检查一遍。
                                    """


                                prepartion=await miniagent_run_once(analyze_agent, PREPARE_PROMPT)
                                print("prepartion:",prepartion)
                                if prepartion!="no need": 
            ...
            if subtask.startswith("push_") and (subtask.endswith("_left") or subtask.endswith("_right")):
                ...
                thing_prompt = THING_PROMPT_TMPL.format(
                        ...
                        prep_text=prepartion,
                        history_path=prompt_log_path,
                    )
            ...
            if subtask=="push_into_drawer":
                DRAWER_RULE_PROMPT = f"""
                    你是“抽屉策略切换判定器（Drawer Mode Switcher）”。

                    ========================
                    【硬性规则（必须严格遵守）】
                    1) 你只能使用我提供的输入：DONE_SUBTASKS、CURRENT_SUBTASK。
                    2) 你绝对不能编造任何不存在的任务名。
                    3) 禁止输出解释、理由、标点、代码块或多余文本。
                    4) 最终输出必须只有一行，且只能是以下两种之一（完全匹配）：
                    - place_in_drawer
                    - NONE
                    ========================
                    ...
                    CURRENT_SUBTASK（一个名字）：
                    {subtask}

                    ========================
                    【判定规则】
                    仅当同时满足：
                    A) DONE_SUBTASKS 中任意位置出现过某个任务名满足：
                    - 以 "push_" 开头
                    - 且以 "_left" 结尾
                    则输出：place_in_drawer
                    ...
                    【输出】
                    只输出一行：place_in_drawer 或 NONE
                    """

                resp3=await miniagent_run_once(drawer_agent, DRAWER_RULE_PROMPT)
                if resp3!="NONE":
                    prompt2=str(task_instructions["place_in_drawer"][0])
                    logger.info(prompt2)
            ...
            while True:
                ...
                for _ in range(args.max_steps):
                    f=_
                    if f>240 and episode_attempt>0:
                        episode_attempt-=1
                        attempt-=1
                ...
                if done:
                    if subtask=="move_slider_left" or subtask=="move_slider_right" or subtask=="open_drawer":
                            condition_subtask=await miniagent_run_once(pre_rule_agent, PRE_RULE_PROMPT)
                            if condition_subtask!="NONE":
                                obs=go_to_init_tcp(env, initial_state=initial_state)
                                action_plan.clear()
                                obs = run_prompt_steps(
                                                        env, policy, obs,
                    ...
                    logger.info(f"real_steps: {f}")
                    if use_thinking_agent:
                        record = {
                                    "subtask": subtask,
                                    "prompt2": str(prompt2),
                                    "outcome": outcome_word,          # "succeeded"/"failed"
                                    "prepare_prompt": prepartion,     # or "no need"
                                }
                        append_jsonl(prompt_log_path, record)
                ...
                #     memeory_txt=await miniagent_run_once(analyze_agent, ANALYZE_PROMPT)
                break
