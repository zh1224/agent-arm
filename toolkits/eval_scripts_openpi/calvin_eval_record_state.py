...
def main(args):
    ...
    import json
    subtask_init_states = collections.defaultdict(lambda: {"success": [], "fail": []})
    ...
    for i, (initial_state, task_sequence) in enumerate(tqdm.tqdm(task_definitions)):
        ...
        for subtask in task_sequence:
            ...
            if done:
                subtask_init_states[pair_key]["success"].append(subtask_init_state)
            else:
                subtask_init_states[pair_key]["fail"].append(subtask_init_state)
    ...
    out_json = pathlib.Path(f"{args.log_dir}/{args.exp_name}/subtask_init_states.json")
    out_json.parent.mkdir(parents=True, exist_ok=True)
    with open(out_json, "w") as f:
        json.dump(subtask_init_states, f, indent=2)
    logger.info(f"Saved subtask init states to: {out_json}")
